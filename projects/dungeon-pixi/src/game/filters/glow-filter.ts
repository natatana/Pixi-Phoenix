...
void main(void) {
    ...
    vec4 ownColor = texture(texture, vTextureCoord);
    vec4 curColor;
    ...
    for (float angle = 0.; angle <= PI * 2.; angle += angleStep) {
       cosAngle = cos(angle);
       sinAngle = sin(angle);
       for (float curDistance = 1.; curDistance <= distance; curDistance += 1.) {
          vec2 offset = vTextureCoord + vec2(cosAngle, sinAngle) * curDistance * px;
          curColor = texture(texture, offset);
    ...
    gl_FragColor = vec4(
        mix(
          mix(ownColor.rgb, glowColor.rgb, innerGlowAlpha / ownColor.a),
          glowColor.rgb,
          outerGlowAlpha / resultAlpha
        ) * resultAlpha,
        resultAlpha
    );
}
...
const GlowFilter = new PIXI.Filter({
    ...
    resources: {
      timeUniforms: {
        ...
        distance: { value: 10, type: 'f32' },
        outerStrength: { value: 1, type: 'f32' },
        innerStrength: { value: 1, type: 'f32' },
        glowColor: { value: [1, 1, 1, 1], type: 'vec4<f32>' },
        pixelWidth: { value: 1, type: 'f32' },
        pixelHeight: { value: 1, type: 'f32' },
      },
    },
  });
